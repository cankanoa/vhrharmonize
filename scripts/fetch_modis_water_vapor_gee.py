...
FIND_FILES_MODULE_PATH = os.path.join(PROJECT_ROOT, "vhrharmonize", "find_files.py")
_spec = importlib.util.spec_from_file_location("vhr_find_files", FIND_FILES_MODULE_PATH)
if _spec is None or _spec.loader is None:
    raise RuntimeError(f"Could not load module from {FIND_FILES_MODULE_PATH}")
_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_module)
find_files = _module.find_files
find_roots = _module.find_roots
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Optional .env file for Earth Engine settings (default: scripts/.env).",
    )
    ...
    parser.add_argument(
        "--output-json",
        default="scripts/modis_water_vapor_results.json",
        help="Output JSON report path.",
    )
    parser.add_argument(
        "--output-csv",
        default="scripts/modis_water_vapor_results.csv",
...
def main() -> int:
    ...
    if not ee_project:
        print(
            ...
            "Set --ee-project or add GEE_PROJECT in scripts/.env."
        )
