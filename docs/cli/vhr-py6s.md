...
```bash
conda install conda-forge::sixs
pip install "vhrharmonize[py6s]"
```

Conda installs the 6S executable and the `py6s` extra installs the Python interface. If the executable is not available on `PATH`, pass its path using `--py6s-executable`.
